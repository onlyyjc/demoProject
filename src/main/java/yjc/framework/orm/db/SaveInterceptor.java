...
public class SaveInterceptor implements Interceptor,Serializable{
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        // 并将其保存到delegate属性里。
        if (invocation.getTarget() instanceof ReuseExecutor) {
            Object[] objects = invocation.getArgs();
            System.out.println(JSON.toJSONString(objects[1]));
            if (objects != null && objects.length>1){
                if (objects[0] instanceof MappedStatement){
...
//            ReuseExecutor reuseExecutor = (ReuseExecutor) invocation.getTarget();
//            Map<String, Statement> delegate =
//                    (Map) getField(ReuseExecutor.class, "statementMap").get(reuseExecutor);
